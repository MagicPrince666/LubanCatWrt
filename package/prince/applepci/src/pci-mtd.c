...
/*
 * Apple PCI nand Flash driver
 */

static int
apple_disk_init(struct pci_dev *dev, struct map_pci_info *map)
{
	u32 win_base;
...
static void
apple_disk_exit(struct pci_dev *dev, struct map_pci_info *map)
{
	if (map->base)
		iounmap(map->base);
...
apple_disk_translate(struct map_pci_info *map, unsigned long ofs)
{
...
static struct mtd_pci_info app_pci_nand_info = {
	.init =		apple_disk_init,
	.exit =		apple_disk_exit,
	.translate =	apple_disk_translate,
	.map_name =	"apple_probe",
};
...
static const struct pci_device_id mtd_pci_ids[] = {
	{
		...
		.driver_data =	(unsigned long)&app_pci_nand_info,
	},
	{ 0, }
};
